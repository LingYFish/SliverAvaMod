...
class ItemSlot(object):
    def __init__(self, screenNode, path, index = 0,canOutput=True,canInput=True,canInputItem=[]):
        self.screenNode = screenNode #type: Inventory
        self.path = path
        self.canOutput = canOutput
        self.canInput = canInput
        self.canInputItem = canInputItem
    ...
    def CanInput(self,itemSlot):
        #type: (ItemSlot) -> None
        if self.canInput and len(self.canInputItem) == 0:
            return True
        elif self.canInput and len(self.canInputItem) > 0:
            if (itemSlot.itemStack.identifier,itemSlot.itemStack.data) in self.canInputItem:
                return True
        return False
    
    def CanOutput(self):
        return self.canOutput
